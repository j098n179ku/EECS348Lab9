...
*/
template <typename T> Matrix<T>::Matrix(size_t N) // initialization for an empty NxN matrix. passes N as a parameter for size
{
...
*/
template <typename T> void Matrix<T>::print_matrix() const //inits the function to print out the matrix
{
...
/*
    ...
    index
*/
template <typename T> void Matrix<T>::set_value(std::size_t i, std::size_t j, T n) //passes through integer n, size_t is an unsigned integer type. this is to help against negative indexes
{
...
*/
template <typename T> T Matrix<T>::get_value(std::size_t i, std::size_t j) const //initializes the get value function. takes i and j as parameter which is the row and column
{
...
*/
template <typename T> int Matrix<T>::get_size() const //intializes the get size function to return the size of the matrix
{
...
*/
template <typename T> T  Matrix<T>::sum_diagonal_major() const //initializes the sum diagonal in the major direction function and takes no parameters
{
...
*/
template <typename T> T Matrix<T>::sum_diagonal_minor() const//initialize the diagonal addition of the minor diagonal. takes no parameters
{
...
*/
template <typename T> Matrix<T> Matrix<T>::operator*(const Matrix<T> &rhs) const //inititalizes the multiplication overload operator. it takes the values of the "other" matrix as a parameter so it can be multiplied
{
...
*/
template <typename T> void Matrix<T>::swap_rows(std::size_t r1, std::size_t r2)//inits the swap rows function, it takes 2 row indexes as parameters. the data inside these will be swapped
{
...
*/
template <typename T> void Matrix<T>::swap_cols(std::size_t c1, std::size_t c2)//initializes the swap columns function. it takes in column 1 and 2 from the users input
{
