...
/*
    The compiler was giving warnings and errors when i changed all these functions to be ternary.
    Turns out the issue was just because the ternary functions have to be all in the .hpp file. so the
    matrix.cpp file basically wasnt used in this project
    source I used to figure out the issue:
    https://stackoverflow.com/questions/495021/why-can-templates-only-be-implemented-in-the-header-file

    but it still works the same as before
*/
...
*/
template <typename T> Matrix<T>::Matrix(size_t N) // initialization for an empty NxN matrix. passes N as a parameter for size. changed this from last lab to have the ternary type T so it can take integers or doubles
{
...
*/
template <typename T> Matrix<T>::Matrix(std::vector<std::vector<T>> nums) //inits the contructor and takes a 2d vector as the parameter. I needed to add the T template in front of the matrix class constructor so it can take both ints and doubles
{
...
*/
template <typename T> void Matrix<T>::print_matrix() const //inits the function to print out the matrix. changed this from last lab to have the ternary type T so it can take integers or doubles
{
...
/*
    ...
    index
*/
template <typename T> void Matrix<T>::set_value(std::size_t i, std::size_t j, T n) //passes through integer n, size_t is an unsigned integer type. this is to help against negative indexes. changed this from last lab to have the ternary type T so it can take integers or doubles
{
...
*/
template <typename T> T Matrix<T>::get_value(std::size_t i, std::size_t j) const //initializes the get value function. takes i and j as parameter which is the row and column. changed this from last lab to have the ternary type T so it can take integers or doubles
{
...
*/
template <typename T> int Matrix<T>::get_size() const //intializes the get size function to return the size of the matrix. changed this from last lab to have the ternary type T so it can take integers or doubles
{
...
*/
template <typename T> T  Matrix<T>::sum_diagonal_major() const //initializes the sum diagonal in the major direction function and takes no parameters. changed this from last lab to have the ternary type T so it can take integers or doubles
{
...
*/
template <typename T> T Matrix<T>::sum_diagonal_minor() const//initialize the diagonal addition of the minor diagonal. takes no parameters. changed this from last lab to have the ternary type T so it can take integers or doubles
{
...
*/
template <typename T> Matrix<T> Matrix<T>::operator+(const Matrix<T> &rhs) const //initializes the matrix addition operator. rhs refers to the second matrix. it is basically the same as using "other" in python for the overlaod operators and takes it as a parameter
{
    Matrix<T> matrixadd(size);//initializes a new matrix that will be used to store the results of the matrix addition
...
*/
template <typename T> Matrix<T> Matrix<T>::operator*(const Matrix<T> &rhs) const //inititalizes the multiplication overload operator. it takes the values of the "other" matrix as a parameter so it can be multiplied. changed this from last lab to have the ternary type T so it can take integers or doubles
{
    Matrix<T> matrixmult(size); //initialization of a new matrix with an NxN of size so it can store the result of the matrix multiplication
...
*/
template <typename T> void Matrix<T>::swap_rows(std::size_t r1, std::size_t r2)//inits the swap rows function, it takes 2 row indexes as parameters. the data inside these will be swapped. changed this from last lab to have the ternary type T so it can take integers or doubles
{
...
*/
template <typename T> void Matrix<T>::swap_cols(std::size_t c1, std::size_t c2)//initializes the swap columns function. it takes in column 1 and 2 from the users input. changed this from last lab to have the ternary type T so it can take integers or doubles
{
