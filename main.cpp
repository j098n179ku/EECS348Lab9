...
int main(int argc, char *argv[])
{
    ...
    int matrix_size = 0; //string that initializes and stores the matrix size so it can be passed to the constructor
    int matrix_data; //this grabs the matrix data from the line so it can be added to the matrix if the data of the matrix is an integer
    ...
    if (!file) //if the file was not found, then let the user know that the fle wasnt able to be opened
    {
        ...
        file >> matrix_size >> type_select; //this command uses the fstream >> in order to grab the matrix size and type selection
        std:: getline(file, line); //this skips the empty line in the text file so the matrix doesnt store non integer values
        
        if (type_select == 0) //if the type select which is found on the first line of the txt file is 0, then that means the matrices will be created with integers in mind
        {
            /*
        ...
        else if (type_select == 1) //if the type select which is found on the first line of the txt file is 1, then that means the matrices will be created with integers in mind
        {
            ...
            for (int i = 0; i < matrix_size; i++) //for each row in the text file that has the matrix data in it
            {
                for (int j = 0; j < matrix_size; j++) //for each column in the text file that has the matrix data in it
                {
                    file >> matrix_double_data; //the >> overload operator is part of fstream. it automatically splits up the data by using whitespace as a delimiter and saves it to the matrix_data to be passed to the constructor
                    matrix3.set_value(i, j, matrix_double_data); //uses the set_value function of the matrix to add the matrix data at that specific index for matrix 1
                }
            }
            ...
            for(int i = 0; i < matrix_size; i++) //starts the for loop for the second matrix, this loop iterates through the rows
            {
                ...
                {
                    file >> matrix_double_data; //the >> overload operator is part of fstream. it automatically splits up the data by using whitespace as a delimiter and saves it to the matrix_data to be passed to the constructor
                    matrix4.set_value(i, j, matrix_double_data);//uses the set_value function of the matrix to add the matrix data at that specific index for matrix 2
                }
            }
        }
        ...
        while (true) //while true allows the menu to be repeatedly used  until the user is done using the program, if the user inputs anything that isnt a choice on the menu, then itll end the program
        {
            try//this try block is used to catch any errors and allows the loop to continue without crashing. its here to catch indexes that are out of bounds or type mismatches like the user trying to enter a string for an integer input
            {
                ...
                /*
                    ...
                    operator created. I modified this so that way it can use either integers or doubles
                */
                if (user_input == 1) //if the user input is 1, it adds the matrices together and prints it out in a new matrix
                {

                    if (type_select == 0) //if the type selection when the document was read was 0, then it will do the operation in terms of integers
                    {
                        std::cout << "\nMatrix 1 + Matrix 2 =\n"; //lets the user know what the result is so the user can easily identify the choice they made
                        Matrix<int> added_matrix = matrix1 + matrix2; //uses the addition overload operator for adding together the first and second matrix
                    ...
                    else if (type_select == 1) //otherwise, if the type selection was 1, then use doubles instead and uses matrix 3 and 4 since they're the ones initialized with doubles
                    {
                        std::cout << "\nMatrix 1 + Matrix 2 =\n"; //lets the user know what the result is so the user can easily identify the choice they made
                        Matrix<double> added_matrix = matrix3 + matrix4; //uses the addition overload operator for adding together the first and second matrix
                        added_matrix.print_matrix(); //prints out the newly made matrix that added together the values of the matrix using matrix addition
                    }
                }
                /*
                ...
                */
                else if (user_input == 2) //if the user input is 2, then it multiplies out the matrices and prints out a new matrix
                {
                    if (type_select == 0) //if the type selection when the document was read was 0, then it will do the operation in terms of integers
                    {
                        ...
                        Matrix<int> mult_matrix = matrix1 * matrix2; //initializes a new matrix and then saves the matrix multiplication table of matrix1 times matrix2
                        mult_matrix.print_matrix();//prints out the newly made matrix made by multiplication
                    }
                    else if (type_select == 1)//otherwise, if the type selection was 1, then use doubles instead and uses matrix 3 and 4 since they're the ones initialized with doubles
                    {
                        std::cout << "\nMatrix 1 * Matrix 2:\n";//lets the user know that the output will be matrix1 times matrix2
                        Matrix<double> mult_matrix = matrix3 * matrix4; //initializes a new matrix and then saves the matrix multiplication table of matrix1 times matrix2
                        mult_matrix.print_matrix();//prints out the newly made matrix made by multiplication
                    }
                }
                /*
                ...
                */
                ...
                {
                    if (type_select == 0) //if the type select is 0, then the function calls all need to be in terms of using the integer valued functions
                    {
                    ...
                    else if (type_select == 1) //if the type select is 0, then the function calls all need to be in terms of using the double valued functions
                    {
                        std::cout << "\nMajor Diagonal Sum of matrix 1: " << matrix3.sum_diagonal_major() << "\n"; //calls the sum_diagonal_major function and prints out the result for matrix 1
                        std::cout << "Minor Diagonal Sum of matrix 1: " << matrix3.sum_diagonal_minor() << "\n";//calls the sum_diagonal_minor function and prints out the result for matrix 2
            
                        std::cout << "Major Diagonal Sum of matrix 2: " << matrix4.sum_diagonal_major() << "\n";//calls the sum_diagonal_major function and prints out the result for matrix 2
                        std::cout << "Minor Diagonal Sum of matrix 2: " << matrix4.sum_diagonal_minor() << "\n";//calls the sum_diagonal_minor function and prints out the result for matrix 2
                    }
                }
                /*
                ...
                */
                else if (user_input == 4)//if the user input is 4 then it will ask the user to pick a matrix, then 2 rows. those two rows will then swap positions
                {
                    ...
                    if(choose_matrix == 1) //if the first matrix was chosen to be swapped
                    {
                        ...
                        {
                            matrix3.swap_rows(row1_input, row2_input);//call the swap rows on matrix2 and pass through the row 1 and row 2 inputs as parameters
                            matrix3.print_matrix();//calls the print function to out the newly swapped matrix2 for the user to show it happened
                        }
                    ...
                    else if (choose_matrix ==2)//if the second matrix was chosen to be swapped
                    {
                        ...
                        {
                            matrix4.swap_rows(row1_input, row2_input);//call the swap rows on matrix2 and pass through the row 1 and row 2 inputs as parameters
                            matrix4.print_matrix();//calls the print function to out the newly swapped matrix2 for the user to show it happened
                        }
                ...
                */
                else if (user_input == 5)//if the user input is 4 then it will ask the user to pick a matrix, then 2 columns. those two columns will then swap positions
                {
                    ...
                    if(choose_matrix_col == 1)//if the matrix chosen by the user is matrix 1, then it will use matrix 1 to call the swap
                    {
                        ...
                        {
                            matrix3.swap_cols(col1_input, col2_input);//calls the swap columns function and passes in the column 1 and 2 input as the parameters
                            matrix3.print_matrix();//prints out the newly modified matrix to show that the modification did happen
                        }
                    ...
                    else if (choose_matrix_col ==2)//otherwise, if the second matrix was chosen to be the one to column swap with
                    {
                        ...
                        {
                            matrix4.swap_cols(col1_input, col2_input);//uses matrix 2 to call the swap column function and passes the column 1 and 2 inputs as parameters
                            matrix4.print_matrix();//prints out the newly modified matrix to show that the modification did happen
                        }
                ...
                */
                else if (user_input == 6)//if the user input is 6, then it will ask the user to either use matrix 1 or 2, then it asks for a row and column, then it updates that matrix at that specific index
                {
                    ...
                    {
                        std::cout << "\nPlease enter a new double data value to replace it with: ";//prompts the user to input a new data for that specific index
                        std::cin >> new_num_double;//saves the user input as new_num so it can replace the old number at the specific index
                    }
                    ...
                    if(choose_matrix_update == 1)//if the users input is one then it chooses the first matrix
                    {
                        ...
                        {
                            matrix3.set_value(row, col, new_num);//calls the set value function to change the value at that specific index
                            matrix3.print_matrix(); //calls the function to print out the updated matrix
                        }

                    }
                    else if (choose_matrix_update ==2)//if the user chooses the second matrix to update
                    {
                        if (type_select == 0) //if the type select is 0, then the function calls all need to be in terms of using the integer valued function
                        {
                        ...
                        {
                            matrix4.set_value(row, col, new_num); //calls the set value function to change the value at that specific indexn
                            matrix4.print_matrix();//prints out the newly modified matrix2 for the user to look at
                        }
